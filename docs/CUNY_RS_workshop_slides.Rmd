...
```{r eval=T, echo=FALSE, out.width='80%',fig.align='center'}
...
```{r eval=T, echo=FALSE, out.width='60%',fig.align='right'}
...
- **Vegetatiob structure**: ALOS Annual Mosaics*
---
class: inverse, center, middle
...
**Other**: `ee.List`, `ee.Number`, `ee.String`, etc.
...
*Reducing*: combining images. This is one of my main uses of GEE: creating image composites!
...
You can export functions from other files in your repository (or shared repositories) to reuse common custom functions.

Let's take the `mask_cloud` function and save it on a separate file called `LScloudmask`. The only difference is that we substitute `var` by `exports.`:
...
*Feature and Feature Collection*: the equivalent of `Image` and `ImageCollection`, for vector data.
