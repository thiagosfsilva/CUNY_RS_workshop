...
---
# Why do remote sensing in `R`?

Seamless transition into data alaysis

Capable libraries for handling raster and vector data

Possibility of automated plotting

# Why not do remote sensing in `R`?

Slooooooow

**My current workflow: pre-process in `GEE` or `GDAL/Python`, analyze in `R`.**
---
# Reading data in:

The `raster` library can handle most of the data I/O, using `rgdal` as a backend.

```{r eval=T}
library(raster)

L8_dry <- brick('~/Projects/CUNY_RS_workshop/L8_imgs/L8_dry_season_2005-2010.tif')
...
```
---
```{r eval=T, out.width='\\textwidth'}
plotRGB(L8_dry,r=6,g=5,b=4, scale=10000)
```
---
